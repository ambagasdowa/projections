				...
				<p style="text-align:center;font-size:160%;">
				  <?php
					if(isset($_SESSION['projections']['decimals'])){
					  e("\$ ").e(number_format($AreaCorp['ingresos_all'], 2, '.', ','));
					}else{
					  e("\$ ").e(number_format(round($AreaCorp['ingresos_all'])));
					}
				  ?>
...
// 			);
		...
		<td colspan="2" style="text-align:center;" />
		    <?php 
		      $today = date('Y-m-d');
		      echo $this->Html->link( 
			   $this->Html->image("thumbs/daily/ingresos_".$AreaCorp['Area']."_".$numMonth."_".$_SESSION['Auth']['User']['year'].".png",
			                array("alt" => "Detalles",
					      'title' => 'Detalles',
				              'width' => '680',
				              'height' => '210'
...
// 		  e($flt[$key][0]);
   ...
   <table id="<?php e(idTotalIndex);?>" class="table table-responsive" >
	  <td style="text-align:center;font-size:12px;font-variant:small-caps;"/>
		  <a href="#" class="btn btn-default" role="button" onclick="Effect.toggle('detallesFlotasIngresos', 'appear'); return false;">
			&#9660; Flotas &#9660;
		  </a>
  </table>
  <div id="detallesFlotasIngresos" style="display:none;">
