...
// 	echo $ajax->link("click me!","/ajax_link/disp",$opt);
// 	echo $this->Ajax->link(back,
// 			array(
...
// 			);
// pr($filter);
$empresa = array('1'=>'bonampak','2'=>'atm','3'=>'teisa');
// pr($id_empresa);
?>
...
// 	    $imageUrl = "thumbs/graph_".$filter['year']."_".$filter['area'].".png";
		$imageUrl = "thumbs/daily/toneladas_".$empresa[$filter['id_empresa']]."_".$_SESSION['Auth']['User']['year'].".png";
// 		pr($imageUrl);
// 		pr($filter);
	  }elseif($filter['area'] == true && $filter['flota'] == false){
	    e('Detalle Mensual');
	    $imageUrl = "thumbs/daily/toneladas_".$filter['areaName']."_".$_SESSION['Auth']['User']['year'].".png";
// 				pr($filter);
...
// 		get the totals for month and fraction
			if(!empty($toneladas[$dt['short']][$data])){
