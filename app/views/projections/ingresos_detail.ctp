...
// 	    $imageUrl = "thumbs/graph_".$filter['year']."_".$filter['area'].".png";
		$imageUrl = "thumbs/daily/ingresos_".strtolower($_SESSION['Auth']['User']['empresa'])."_".$_SESSION['Auth']['User']['year'].".png";
// 		pr($imageUrl);
// 		pr($filter);
	  }elseif($filter['area'] == true && $filter['flota'] == false){
	    e('Detalle Mensual');
	    $imageUrl = "thumbs/daily/ingresos_".$filter['areaName']."_".$_SESSION['Auth']['User']['year'].".png";
// 				pr($filter);
...
// 		get the totals for month and fraction
			  if(!empty($SumIngress[$dt['short']][$data])){
				  $num = money_format('%i',$SumIngress[$dt['short']][$data]);
